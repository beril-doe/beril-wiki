...
corpus has 81. Each justified create goes through compile.py's normal
...
concepts are never touched here (compile owns merges), so re-running is safe.
    ...
    OPENAI_API_KEY=$CBORG_API_KEY OPENAI_BASE_URL=https://api.cborg.lbl.gov \
        uv run python pipeline/enrich_concepts.py [--root DIR]
"""
...
{{"create": [{{"name": "concept-slug", "title": "Title",
  "justification": "why no existing page covers this phenomenon; name the closest existing page and why it is not the same thing"}}]}}
...
def main(root: pathlib.Path) -> int:
    ...
    for spage in sorted((wiki / "summaries").glob("*.md")):
        ...
        plan_raw = C.llm(
            [{"role": "system", "content": system},
             ...
             {"role": "user", "content": ENRICH_PLAN_USER.format(
        ...
        for it in plan.get("create") or []:
            ...
            task = C.CONCEPT_CREATE_USER.format(title=it.get("title") or name, name=name,
                                                sid=sid, summary_stem=stem)
            try:
                obj = C.generate_page(
                    [{"role": "system", "content": system},
                    ...
                    f"{sid}/enrich/{name}", sources, targets)
    ...
    print(f"enrich: {created} concept(s) created, {skipped} summaries unchanged, "
          f"{len(C._failures)} failure(s); tokens in={C._usage['in']} out={C._usage['out']} (~${est:.2f} est)")
