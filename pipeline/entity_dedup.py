...
both pages are kept.
    ...
    uv run python pipeline/entity_dedup.py              # report, change nothing
    uv run python pipeline/entity_dedup.py --apply      # merge the pairs found
"""
...
import compile as C
from consolidate_concepts import apply_merge
...
def load(kb: pathlib.Path) -> list[dict]:
    ...
    for f in sorted((kb / "wiki" / "entities").glob("*.md")):
        ...
        pages.append({
            "stem": f.stem, "title": title, "type": (fm.get("type") or "").lower(),
            "key": normalise(title), "slug_key": normalise(f.stem),
            "aliases": aliases(body), "ids": identifiers(body),
...
def duplicate_pairs(pages: list[dict]) -> list[tuple[str, dict, dict]]:
    ...
    for i, a in enumerate(pages):
        for b in pages[i + 1:]:
            ...
            elif a["key"] and a["key"] in (b["key"], b["slug_key"]) or (
                    b["key"] and b["key"] == a["slug_key"]):
                out.append(("name", a, b))
...
def main() -> int:
    ...
    print(f"entity_dedup: {len(pages)} entity pages, {len(pairs)} identified duplicate pair(s)")
    for signal, a, b in pairs:
        # The page citing more projects survives; a tie keeps the shorter name.
        keep, drop = ((a, b) if (a["sources"], -len(a["stem"])) >= (b["sources"], -len(b["stem"]))
                      else (b, a))
        print(f"  [{signal}] {drop['stem']} -> {keep['stem']} "
    ...
    if not sources:
        print(f"entity_dedup: no staged docs under {args.root}/staging", file=sys.stderr)
        return 1
    system = C.SYSTEM.format(
        contract=(C.REPO / "contract" / "AGENTS.md").read_text(encoding="utf-8"))
    ...
    for signal, a, b in pairs:
        ...
        if not apply_merge(args.root, drop["stem"], keep["stem"], targets, sources, system,
                           state, state_path, [], collection="entities",
    ...
    print(f"entity_dedup: {len(pairs) - len(failed)} merged, {len(failed)} refused "
          f"(~${est:.2f} est)")
    for f in failed:
