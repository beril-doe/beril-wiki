...
from entity_dedup import duplicate_pairs, identifiers, load, normalise
...
def _pairs(**pages):
    ...
    return {(min(a["stem"], b["stem"]), max(a["stem"], b["stem"]), sig.split()[0])
...
def test_different_types_never_merge():
    assert _pairs(**{"copper": dict(title="Copper", type_="Compound"),
                     "copper-org": dict(title="Copper", type_="Organism")}) == set()


def test_two_genes_of_one_organism_do_not_merge():
    """The exact pair embeddings rank at 0.971."""
    assert _pairs(**{"aciad2176": dict(title="ACIAD2176", type_="Gene_Or_Pathway"),
                     "aciad3137": dict(title="ACIAD3137", type_="Gene_Or_Pathway")}) == set()
...
def test_a_mentioned_accession_is_not_an_identifier():
    ...
    assert _pairs(**{"klebsiella": dict(title="Klebsiella", type_="Organism", body=body),
                     "methanococcus": dict(title="Methanococcus", type_="Organism", body=body)}) == set()


def test_an_identifier_in_the_identity_section_does_match():
    body = "## Identity\n\nStable external identifier: CHEBI:29036.\n"
    got = _pairs(**{"copper": dict(title="Copper", type_="Compound", body=body),
