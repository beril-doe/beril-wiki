#!/usr/bin/env python3
"""Parity acceptance test for the first-party compiler (DESIGN.md protocol).
...
Then runs wiki_check.py on both and prints which pages our compiler
created/updated. Requires a populated staging/ (run fetch_reports.py first)
...
def setup() -> None:
    ...
    for name in ("ours", "ref"):
        dest = PARITY / name
        for sub in ("wiki", "wiki-extra"):
            shutil.copytree(REF / sub, dest / sub)
        os.symlink(REPO / "staging", dest / "staging")

    ours = PARITY / "ours"
    hashes = {}
    for f in sorted((REPO / "staging").glob("*.md")):
...
def run_check(root: pathlib.Path, label: str) -> str:
    r = subprocess.run([sys.executable, str(HERE / "wiki_check.py"), str(root)],
...
def main() -> int:
    if not (REPO / "staging").is_dir() or not any((REPO / "staging").glob("*.md")):
        raise SystemExit("parity: run fetch_reports.py first (staging/ is empty)")
    setup()
    ...
    r = subprocess.run(
        [sys.executable, str(HERE / "compile.py"), "--root", str(PARITY / "ours"), *docs])
    ...
    updated = sorted(p for p in after if p in before and after[p] != before[p]
                     and not p.startswith("wiki/index") and not p.startswith("wiki/log"))
    print(f"\n== pages: {len(created)} created, {len(updated)} updated (index/log excluded)")
    for p in created:
        print(f"  created  {p}")
    for p in updated:
        print(f"  updated  {p}")
