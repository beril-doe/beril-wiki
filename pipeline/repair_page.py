...
def canonicalise_sources(path: pathlib.Path, dry_run: bool) -> bool:
    ...
    Deterministic and free — no model call. DESIGN.md's corpus contract says
    ...
    print(f"  {path.relative_to(C.REPO)}: dropping {dropped} uncited source(s) from frontmatter")
    if not dry_run:
...
def main() -> int:
    ...
    print(f"repair_page: {fm_fixed} frontmatter fix(es), {verb} {repaired} page(s), "
    ...
    # than reach wiki_check as a non-blocking warning and then publish.
