...
then prove parity. Everything else in `src/beril_wiki/` already works and carries
over with path adaptations. Scope of this build:

1. `src/beril_wiki/compiler.py` — the compiler (spec below).
2. `src/beril_wiki/stages/fetch.py` — input sync (spec below).
3. Adapt the existing stages' paths to this repo's layout.
...
## Compiler spec (`src/beril_wiki/compiler.py`)
...
2. **Plan** — one call given: the new summary, the full concept/entity index
   ...
   current output (reuse logic from `src/beril_wiki/check.py`). Returns JSON:
...
4. **Write-time validation with retry (the core upgrade over OpenKB)** —
   ...
   source; every flagged number (reuse `check.py`'s NUMBER regex) must
...
## Input (`src/beril_wiki/stages/fetch.py`)
...
- `local` (now): sync `REPORT.md` per project + `docs/discoveries.md` +
  ...
  `stages/figures.py` and `publish/ingest.py` — adapt their `REPO/projects`
  paths to this).
...
- Layout: `wiki/{summaries,concepts,entities}/*.md`, `wiki/{topics,conflicts,authors,data}/*.md` + `wiki/index.md` (home).
...
- Frontmatter on wiki/ pages: `type`, `description`, `sources` (list of
  ...
  body is the real measure; `stages/consolidate.py` enforces the two in step.
- Downstream stages (`stages.conflicts`, `stages.topics`, `stages.figures`,
  `stages.extras`, `check`, `publish.ingest`) all parse these conventions —
...
2. Judge: `check.py` (0 errors required; warnings ≤ reference), merge
...
- Every stage leaves a runnable check; match the existing code style in
  `src/beril_wiki/` (it is the house style).
