...
#
#   CBORG_API_KEY=... scripts/run_pipeline.sh      # full run
#   scripts/run_pipeline.sh --no-publish           # skip the Quartz build
#
...
#
# scripts/run_pipeline.sh --force        rebuild every derived stage, ignoring caches
# scripts/run_pipeline.sh --no-publish   skip the Quartz build
set -euo pipefail
...
echo "== fetch" | tee "$LOG"
"${PY[@]}" -m beril_wiki.stages.fetch | tee -a "$LOG"

echo "== compile" | tee -a "$LOG"
"${PY[@]}" -m beril_wiki.compiler | tee -a "$LOG" | tail -3
if grep -qE "\[ERROR\]" "$LOG"; then echo "compile had errors — see $LOG"; exit 1; fi

echo "== enrich" | tee -a "$LOG"
"${PY[@]}" -m beril_wiki.stages.enrich | tee -a "$LOG" | tail -3

echo "== consolidate" | tee -a "$LOG"
"${PY[@]}" -m beril_wiki.stages.consolidate | tee -a "$LOG" | tail -3
...
"${PY[@]}" -m beril_wiki.stages.entities --apply | tee -a "$LOG" | tail -3

echo "== conflicts" | tee -a "$LOG"
"${PY[@]}" -m beril_wiki.stages.conflicts ${FORCE} | tee -a "$LOG"

echo "== hubs" | tee -a "$LOG"
"${PY[@]}" -m beril_wiki.stages.topics ${FORCE} | tee -a "$LOG"
...
"${PY[@]}" -m beril_wiki.stages.authors | tee -a "$LOG" | tail -3
...
"${PY[@]}" -m beril_wiki.stages.repair | tee -a "$LOG" | tail -3

echo "== check" | tee -a "$LOG"
"${PY[@]}" -m beril_wiki.check "$REPO" | tee -a "$LOG"
