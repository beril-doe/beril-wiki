...
## conflicts/conflict--composite-resistance-score-limitations--4a1c9395.md
- A side-by-side bar chart comparing the two bridges' totals (37,368 strains / 5,647 species vs 42,227 strains / 6,426 species) would make the magnitude of the discrepancy directly visible; no candidate figure shows both pipelines' counts together.

## conflicts/conflict--composite-resistance-score-limitations--a9245bd1.md
...
## conflicts/conflict--functional-marker-validation--6c57ba85.md
- A side-by-side per-genome positive/negative table (bacillota_b_subsurface_accessory vs clay_confined_subsurface) for the retained sulfur-reduction signal would let readers see directly whether the same genomes drive the 5/9 count in both reports, which is the key unresolved question on this tension 
...
## conflicts/conflict--genome-wide-versus-locus-specific-ecological-adaptation--29fec74b.md
...
## conflicts/conflict--shared-stress-versus-stressor-specific-fitness--e9b25de2.md
...
## summaries/adp1_deletion_phenotypes__REPORT.md
- Pairwise condition correlation matrix (28 condition pairs, median r=0.25, max r=0.58 acetate–butanediol) — only described in prose; a correlation heatmap would let readers see which condition pairs are more/less independent.
- Condition-specificity score distribution for the 625 condition-specific genes (score ≥1.0) mapped to pathway/gene identity — the dense per-pathway gene list (pcaC, pcaG, pcaH, etc.) and quinate-specific 51-gene set are described only as lists, not visualized.

## summaries/adp1_triple_essentiality__REPORT.md
- No figure covers the RB-TnSeq vs. knockout essentiality discordance data (paras 9-13): Cohen's kappa across five thresholds (-0.139 to -0.014), the 2x2 concordance breakdown at threshold 0.05 (18/1411/211/293 genes), and the AUC contrast between continuous fitness (0.700/0.725) vs. binary essentiali
- No figure covers the proteomics expression comparison (para 15-16): essential (n=464, mean log2=28.43±2.94) vs. dispensable (n=1824, mean log2=25.73±2.96) genes — a boxplot/violin of log2 expression by essentiality class would support this 6.5-fold difference claim.

## summaries/alphafold_msa_annotation__REPORT.md
- Spearman correlation scatter/binned plot of MSA depth vs. domain hit count and distinct IPR families (ρ=0.7563 across 38,051,842 pairs) — the monotone relationship across MSA-depth bins (paragraphs 9-10) is described only in prose/table form

## summaries/amr_environmental_resistome__REPORT.md
- Table of deeply sampled case-study species (paragraph 14: K. pneumoniae, S. aureus, S. enterica, etc.) with genome counts, AMR cluster counts, core/accessory split, and clinical-dominance percentage has no visual — a grouped bar chart would show the pattern across species.

## summaries/amr_fitness_cost__REPORT.md
- Bar chart of experiment counts by category (2,868 carbon/nitrogen, 1,862 stress, 727 standard, 447 metal, 443 antibiotic; total 6,804) to visualize the data scope described only in prose in paragraph 21.

## summaries/amr_pangenome_atlas__REPORT.md
- Bar chart of AMR clusters per species by environment category (Human/Clinical 10.6, Soil/Terrestrial 4.6, Aquatic 3.9, Animal 3.0) with core-fraction overlay, to visualize the clinical-vs-environmental AMR burden and conservation gradient described in paragraphs 16-19.
- Distribution/violin plot of fitness measurements comparing AMR genes vs non-AMR baseline (median fitness values across 29,386 measurements, 178 AMR genes, 37 Fitness Browser organisms) to visualize the 'no fitness cost' finding in paragraphs 20-22.

## summaries/amr_strain_variation__REPORT.md
- Mantel r distribution comparing core vs non-core AMR genes (median 0.222 vs 0.117, paired t=-8.35) referenced in prose but covered only partially by figure 2 — a paired distribution plot would clarify the effect size
- Island mechanism counts across 8 categories (Other/Unclassified 1026, Efflux 954, Enzymatic inactivation 698, etc.) described only in prose with no bar chart

## summaries/aromatic_catabolism_network__REPORT.md
- Per-condition Complex I ortholog fitness values across substrates (acetate -1.55, succinate -1.39, plus glucose and lactate reported as dispensable) — described only in prose (paragraph 19) with no bar chart or distribution plot showing the substrate-by-substrate comparison

## summaries/bacdive_metal_validation__REPORT.md
- A bar chart or table of metal-score deltas by contamination category (heavy metal +1.00, waste/sludge +0.57, all-contamination +0.43, industrial +0.20) with sample sizes and p-values would visualize the dose-dependent gradient described only in prose (paragraph 6).
- A phylum-stratified comparison chart (Pseudomonadota delta=+0.040, Actinomycetota delta=+0.035, null results for Bacillota/Bacteroidota, with n's) would visualize the phylum-level breakdown described only in prose (paragraphs 9-10).

## summaries/bacillota_b_subsurface_accessory__REPORT.md
- Bar chart comparing anchor vs baseline genome size, GC content, OG counts, and CheckM completeness (means with Cohen's d and p-values) — reported only as prose numbers across paragraphs 9-11
- Grouped bar chart of corrected vs original multi-heme cytochrome positive-genome percentages across anchor_deep, anchor_shallow, and soil_baseline cohorts — the reversal of the original iron-reduction contrast is described only numerically in paragraph 15-16

## summaries/berdl_data_atlas__REPORT.md
- The UC1 validation section (paragraphs 20-21) reports a five-way breakdown of the 55,454-gene cohort by fitness-defect category (6,635 essential, 8,271 strong-defect, 10,950 moderate, 29,467 mild, 131 no-defect) with no corresponding chart among the candidates — a bar chart of this distribution woul

## summaries/caulobacter_fur_lipida_loss__REPORT.md
- ChvI regulon phase-structure partition (20 unique-to-early, 10 both-phases, 49 late-consequence genes out of 79 disjoint ChvI-induced genes; paragraphs 9-11) is described only in prose counts — a Venn/partition diagram or grouped bar chart of per-gene logFC by phase would visualize the overlap struc
- Peptidoglycan remodeling locus-level results (28 of 53 preregistered loci meeting threshold, mix of transcript FDR<0.05 and protein |log2|>1, 20 DOWN vs specific induced loci like SdpA +4.8, Pal +2.08/+2.84; paragraphs 17-18) are enumerated only in prose — a transcript-vs-protein log2 scatter or hea

## summaries/cf_formulation_design__REPORT.md
- Bootstrap 95% composite-score confidence intervals for k=1 through k=5 formulations (paragraph 14) — a table/plot of interval widths would show visually that k=2 through k=5 overlap while k=1 is a degenerate point estimate.
- Core-species profile table (paragraph 16: inhibition %, engraftability, pangenome genome count, conserved pathways, lung genome count for each of the 5 species) — currently only in dense prose, would benefit from a comparison table/bar chart.
- PA14 vs core-commensal genomic pathway completeness for the 6 prebiotic-target pathways (myoinositol, xylitol, xylose, arabinose, fucose, rhamnose; paragraph 19) — no figure shows this side-by-side comparison that motivates the prebiotic strategy.

## summaries/clay_confined_subsurface__REPORT.md
- The within-Bacillota_B breakdown of WL/NiFe/SR presence (deep vs baseline counts and p/p_BH values per marker, paragraph 10) is a dense multi-marker table with no corresponding figure — a grouped bar chart of presence proportions by marker and cohort would clarify the phylum-driven vs cohort-level d

## summaries/cofitness_coinheritance__REPORT.md
- Per-organism co-fitness/pangenome extraction table (para 22-23: genome counts, cluster counts, and cofit pair counts across 11 species) is dense and would benefit from a summary table or bar chart rather than prose.

## summaries/conservation_fitness_synthesis__REPORT.md
- Function-specific core-burden excess percentages across gene categories (motility/chemotaxis +7.8pp, RNA metabolism +12.9pp, protein metabolism +6.2pp, and the reversed pattern in cell-wall genes) are given only as numbers in prose (paragraph 13) and would benefit from a bar chart comparing core vs.

## summaries/conservation_vs_fitness__REPORT.md
- Distribution of per-organism essential-gene percentages (range 12.9-28.9% across 33 organisms) — reported only as a range in prose, no figure shows the per-organism spread

## summaries/ecotype_functional_differentiation__REPORT.md
- A table of per-species ecotype detection statistics (genome count, number of ecotypes, genomes assigned, silhouette score) across all 12 successful species (paragraph 6) is reported only as dense prose and would benefit from a summary table/bar chart.
- The full per-COG-category table of significance rates and mean effect sizes across all 23 categories (paragraph 15) is given only as prose; a combined bar/scatter of rate vs effect size per category would make the L/S-category standouts visually clear.

## summaries/enigma_carbon_census_1__REPORT.md
- A boxplot or dot plot comparing callable vs. dark compounds on Complexity, MolecularWeight, and HeavyAtomCount (paragraph 27's medians/p-values) has no visual candidate.
- A bar chart of chemical-class counts (Shikimates, Alkaloids, Terpenoids, Fatty acids, Polyketides, AA/Peptides) used in the H1 chi-square test (paragraph 9) has no visual candidate.

## summaries/enigma_contamination_functional_potential__REPORT.md
- Distribution of clade-count-per-genus in the taxonomy bridge (paragraph 19: long right tail, max 433 clades for pseudomonas, 378 for streptomyces, etc.) is described only in prose with no candidate figure covering it.

## summaries/enigma_sso_asv_ecology__REPORT.md
- The metabolic guild co-occurrence results (11 guilds; e.g., nitrifier×iron oxidizer ρ=+0.95, fermenter×predator ρ=+0.85, denitrifier×syntroph ρ=−0.67) are reported only as a list of pairwise correlations — a guild co-occurrence heatmap/correlation matrix would visualize this structure, and no candid
- Groundwater-vs-sediment taxon enrichment (fold-changes for Rhodanobacter, Gallionella, etc. across 5 wells) is described only in prose/text pairs; a bar chart of enrichment ratios by taxon would support this comparison and is not covered by any candidate figure.

## summaries/env_embedding_explorer__REPORT.md
- Metadata field coverage percentages (lat/lon, geo_loc_name, isolation_source, host, env_broad_scale, env_local_scale, env_medium across 83,287 genomes) described only in prose in paragraphs 14-16 — a bar chart of coverage per field would make the uneven harmonization pattern legible.

## summaries/essential_genome__REPORT.md
- Distribution/histogram of essentiality penetrance across the 4,799 variably essential families (median 33%; 813 families >50% essential, 704 <10% essential) — no figure visualizes this distribution.
- Scatter plot of essentiality penetrance vs. core fraction (rho=0.123, p=1.6e-17; >80% penetrance families 97.1% core vs 92.8% for lower-penetrance families) — correlation described only in prose.

## summaries/essential_metabolome__REPORT.md
- Carbon-source utilization matrix across the 7 organisms (which carbon sources are conserved vs. organism-specific, e.g. ethanol/deoxyinosine present in only 6/7) is described only in prose and has no supporting figure/table.

## summaries/euk_in_prok_correlates__REPORT.md
- A site-level breakdown of median eukaryotic fraction across the 47 NEON geography sites (H=310.4, p=2.4×10⁻⁴⁶), highlighting the Arctic tundra outliers (Utqiaġvik 30%, Caribou-Poker Creeks 22%, Toolik 17%) versus temperate forest sites (2–8%), is not visualized by any candidate figure.

## summaries/field_vs_lab_fitness__REPORT.md
- The six-category breakdown of 757 experiments (lab-nutrient 237, field-core 204, lab-other 140, field-stress 78, heavy-metals 55, lab-antibiotic 43) is described only in prose (paragraph 7) and would benefit from a bar chart of experiment counts/percentages by category.
- The sensitivity analysis across fitness thresholds -1 to -3 for field-stress, field-core, and lab-nutrient core fractions (paragraph 13) is a multi-threshold trend reported only as numbers in text, and no candidate figure shows this line/threshold trend.

## summaries/fitness_effects_conservation__REPORT.md
- Ephemeral-niche gene enrichment across conservation categories (core 3.0%, auxiliary 1.7%, singleton 1.6%; n=4,450 total) is reported only as prose percentages — a bar chart comparing these three rates would visualize the gradient no candidate figure covers.

## summaries/functional_dark_matter__REPORT.md
- Darkness spectrum tier distribution (T1 Void 4,273; T2 Twilight 12,282; T3 Dusk 16,103; T4 Penumbra 22,500; T5 Dawn 1,853) — no figure visualizes this five-tier classification of all 57,011 dark genes (para 27).
- Phylogenetic species-count distribution for dark-gene clusters (median 135, range 1–27,482 species after GTDB r214 expansion vs. median 1, range 1–33 under initial eggNOG classification) — a histogram/violin comparing the two classifications is described only in prose (paras 12–13).
- Top-100 candidate organism breakdown (22 organisms; Shewanella MR-1 contributing 25 candidates) and phylum composition of top 500 candidates (417 Gammaproteobacteria, 71 Alphaproteobacteria, 17 Deltaproteobacteria, 12 Bacteroidetes, 10 Betaproteobacteria, 5 Firmicutes, 2 Cyanobacteria) — no bar char

## summaries/gene_function_ecological_agora__REPORT.md
- A bar/dot chart of recent-vs-ancient gain percentages and recent:ancient ratios across function classes (CRISPR-Cas 58.7%/2.4%, TCS histidine kinases 45.1%/4.4%, β-lactamases 44.2%/4.9%, etc., from paragraph 8) — this dense cross-class comparison has no visual support among the candidate figures.
- A grouped bar chart of BacDive phenotype proportions and AlphaEarth environmental-cluster percentages across the three focal clades (paragraphs 14-15: e.g. Mycobacteriaceae rod-shape 95%/non-motile 99%/aerobic 89.4%; cluster percentages by clade) — currently only described in prose with no distribut

## summaries/genotype_to_phenotype_enigma__REPORT.md
- A bar/scatter distribution of per-condition-class AUC values (amino acids 0.775, nucleosides 0.780, etc. across 343 conditions with 95 exceeding 0.75) is described only in prose (paras 12-13) and would benefit from a chart.
- The KO-presence/absence vs. µmax/lag/yield weak-correlation results (para 14, e.g. n_unique_KOs vs µmax r=+0.42) are reported only as prose correlation coefficients with no scatterplot support.

## summaries/harvard_forest_warming__REPORT.md
- PERMANOVA variance partition (treatment 7.6% p=0.069, horizon 30.6% p=0.0002, treatment×horizon 41% p=0.0002) has no visual — a stacked/bar chart of variance components by factor would clarify the dominance of horizon and interaction over treatment alone.

## summaries/ibd_phage_targeting__REPORT.md
- The six actionable Tier-A targets and their total scores (Hungatella hathewayi 4.0, Mediterraneibacter gnavus 3.8, Escherichia coli 3.6, Eggerthella lenta, etc., paragraph 10) are reported only as a prose list — a bar chart ranking targets by score would make this key prioritization result legible.
- The CLR-Spearman-Louvain co-abundance network statistics (3–7 modules per subnet, 28,730/15,354/30,453/19,909 edges across E1_all/E1_CD/E3_all/E3_CD, paragraph 11) are described only numerically with no network diagram or module-size distribution among the candidates.

## summaries/lab_field_ecology__REPORT.md
- A table/plot of the per-genus Spearman correlation statistics (rho, p, q) for all 11 tested Fitness Browser genera vs uranium would help readers see the full FDR-corrected result set at a glance, since only aggregate/selected genera are discussed in prose.

## summaries/lanthanide_methylotrophy_atlas__REPORT.md
- The PQQ-biosynthesis annotation-gap breakdown (paragraphs 24-25: complete/partial eggNOG pqq calls vs Bakta-only strong/partial evidence vs no-evidence counts across 2,320 xoxF-bearing genomes) is a dense categorical table with no corresponding figure and would benefit from a stacked-bar or Venn vis

## summaries/lignin_community_enrichment__REPORT.md
- Table of 16S alpha-diversity metrics (observed OTUs, Shannon, Pielou) across all 7 groups with SE (paragraph 21) has no figure — a grouped bar/line chart would visualize the diversity collapse trend.
- CLR effect sizes for lignin-associated genera across Base-vs-L and L-vs-LC comparisons (paragraphs 15-16) are described only in prose/lists — a bar chart of CLR effect sizes per taxon would clarify which genera respond to which condition.

## summaries/metabolic_capability_dependency__REPORT.md
- Distribution of latent-capability fraction across the 48 organisms (range 0–31.6%, mean 15.0%, with P. syringae and K. michiganensis highest) has no supporting chart — a bar or dot plot per organism would visualize this variation.
- Per-species silhouette scores for metabolic ecotype clustering (10 target species, range 0.35–0.89) are reported only as text/table with no bar chart to compare across species.

## summaries/metal_cross_resistance__REPORT.md
- The three-tier gene architecture (1,484 general-stress / 2,306 metal-shared / 4,372 metal-specific genes and their 92.0%/91.0%/89.8% pangenome core fractions, para 12) is reported only as numbers — a grouped bar chart of gene counts and core-fraction percentages per tier would make this gradient vis
- The ranked list of consensus metal-pair correlations (Fe-Zn r=0.61, Co-Ni r=0.56, Co-Zn r=0.52, etc., para 6) is a dense table in prose with no corresponding figure; a sorted bar chart of mean r by metal pair (with n) would visualize the magnitude ranking.

## summaries/metal_fitness_atlas__REPORT.md
- Distribution of pangenome-scale metal functional signature scores across the 27,702 scored species (e.g. percentile rank histogram), since only two example organisms (Leptospirillum, Acidithiobacillus) are named in prose with no figure showing the overall score distribution.

## summaries/metal_specificity__REPORT.md
- Per-metal specificity breakdown table (paragraph 7: metal-specific fractions for manganese, molybdenum, tungsten, selenium, cadmium, copper, cobalt, chromium, etc.) is described only in prose and would benefit from a bar chart by metal.
- Candidate specificity table for novel gene families (paragraph 15-16: UCP030820, YebC, DUF1043/YhcB, RapZ, MlaD, YfdZ, YrbC with % metal-specific across organisms and mean sick rates) is dense and enumerated only in text; a scatter/bar comparison would clarify prioritization.

## summaries/microbeatlas_metal_ecology__REPORT.md
- The leave-one-metal-out and leave-one-environment-out sensitivity results (paragraphs 19-20: 7 metal exclusions and 13 environment exclusions, each with its own β, SE, p) are reported only as dense prose lists with no dedicated figure — a forest/bar plot of β with CI across all exclusions would make

## summaries/nmdc_community_metabolic_ecology__REPORT.md
- A table/plot of the 17/18 amino-acid pathway Kruskal-Wallis results (H and q values per pathway, e.g. glycine H=92.98, asparagine H=66.19, cysteine H=62.66, etc.) described only in prose in paragraph 15 has no supporting figure.

## summaries/paperblast_explorer__REPORT.md
- Domain-level breakdown of genes/organisms (Bacteria 5,997 organisms/397,544 genes 35.0%; Unknown 14,798 organisms/397,511 genes 35.0%; Eukarya 822 organisms/290K+ genes) has no chart — a stacked bar or pie by domain would visualize this distribution.
- PDB site-record type breakdown (binding 1.69M, functional 182K, modified 112K, mutagenesis 104K) is reported only as counts in prose with no supporting chart.

## summaries/pathway_capability_dependency__REPORT.md
- A per-genus breakdown of the pathway-variability/pangenome-openness correlation (rho and p-value for each of the 18 genera tested, positive in 13/18, significant in 5/18) is described only in prose (para 15) and has no covering figure.

## summaries/phb_granule_ecology__REPORT.md
- Family-level PHB enrichment/depletion table (248 families tested with Fisher's exact + Bonferroni: 41 enriched, 62 depleted, 145 non-significant) has no supporting figure — a volcano/bar plot of family-level enrichment odds ratios vs significance would visualize this dense result.
- NMDC metagenomic correlations (PHB inference score vs depth, temperature, max/min depth, pH, ammonium) are reported only as a list of rho/p values in prose with no scatter or correlation-matrix figure to show these relationships.

## summaries/plant_microbiome_ecotypes__REPORT.md
- A bar/table of host-species counts (rice 286, Arabidopsis thaliana 184, wheat 122, maize 98, plus the other 15 of 19 testable hosts) is described only in prose and would benefit from a distribution chart.
- The cluster-robust GLM marker odds ratios (ACC deaminase OR=8.01, T3SS OR=2.71, nitrogen fixation OR, and the other markers among the 8/14 significant at BH-FDR q<0.05) are reported as a dense list with no accompanying forest-plot or bar-chart figure.

## summaries/prophage_amr_comobilization__REPORT.md
- The five phylum-specific Spearman rho values for H2 (Pseudomonadota 0.54, Bacillota_A 0.55, Bacillota 0.40, Bacteroidota 0.59, Actinomycetota 0.29) are reported only as text in paragraph 13; a per-phylum bar chart of rho would visualize this cross-phylogenetic consistency claim.

## summaries/prophage_ecology__REPORT.md
- Module co-occurrence statistics (para 24-27): per-module significant-test fractions and mean Z-scores/co-localization (e.g., packaging 11/15 tests, head morphogenesis 1/6, tail 5/12, integration 1/15) across 15 phylogenetically stratified species lack a chart — a grouped bar/heatmap of module x spec

## summaries/pseudomonas_carbon_ecology__REPORT.md
- A table/plot of the 43 significantly different pathways (of 62) with their q-values and completeness percentages by species group is described only in prose (para 5-7) and has no dedicated figure beyond the aggregate heatmap.
- Random Forest per-class accuracy and confusion matrix across the four environment classes (soil/freshwater/plant_surface/rhizosphere) with class sizes (13/13/etc.) is reported only as a single balanced-accuracy number; a confusion matrix or per-class bar chart would visualize this distribution.

## summaries/respiratory_chain_wiring__REPORT.md
- A bar/table visualization of proteomics protein-level percentiles for Complex I (27.6, 66th pct), NDH-2 (27.0, 59th pct), and ACIAD3522 (26.2, 48th pct) vs. genome median (26.4), supporting the 'similar protein levels' claim in finding 5.
- A table or bar chart of the 62-gene respiratory-chain inventory broken down by subsystem (Complex I 13, NDH-2 1, NADH-flavin oxidoreductases 5, cytochrome bo3 4, cytochrome bd 5, Complex II 5, ATP synthase 9, other 20) to visualize the subsystem composition described in paragraph 22.

## summaries/truly_dark_genes__REPORT.md
- A bar chart or table of database-recognition rates for truly dark genes across sequence databases (UniRef50 79.4%, cross-references 84.7%, Pfam domains 4.0%, KEGG KOs 4.6%, eggNOG-mapper 43.5%, COG categories 17.0%) would visualize the sharp contrast between sequence-similarity recognition and funct

## summaries/webofmicrobes_explorer__REPORT.md
- The ModelSEED compound-link quality breakdown (69 exact matches, 107 formula-only matches, 81 unmatched out of 257 identified compounds) is reported only as percentages in prose and would benefit from a simple bar/pie chart of match categories.
