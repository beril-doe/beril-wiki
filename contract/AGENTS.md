...
- **Name the platform correctly**: the data platform is the **KBase Data Lakehouse**. Source reports written earlier call it BERDL, the BER Data Lakehouse, or the KBase BER Data Lakehouse; all three are the same system under names it no longer uses. Never carry those names into a generated page, even when quoting a report's phrasing. Two things are not the platform and keep their names: the project id `berdl_data_atlas` (a filename, a slug and a citation key) and that project's own title, "BERDL Data Atlas". `src/beril_wiki/stages/names.py` sweeps up what slips through, but a page that never says it needs no sweeping.
