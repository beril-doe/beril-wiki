...
**The correlation-scale difference is unattributed.** Resolving whether the correlation-scale difference is driven mainly by downsampling, genome-set composition, extraction behavior (how gene-content features are pulled from each genome record), embedding coverage (how much of a genome's gene content is represented in the numerical feature vectors used), condition coverage (how many assay conditions a species has measurements in), missingness (absent entries in the feature or measurement matrix), or another methodological factor requires a controlled reanalysis. [src: ecotype_env_reanalysis, ecotype_analysis, core_gene_tradeoffs] Until then, no project in the corpus can claim which factor dominates.
