...
Two projects in this corpus reach opposite-looking conclusions about whether environmental context shapes microbial gene content, a tension recorded on [[concepts/ecotype-environment-gene-content]]. One reports a large, highly structured environmental signal; the other reports a null. The disagreement matters because much of the corpus's reasoning about ecotypes — the idea that lineages differentiate functionally along environmental gradients — depends on whether environment is a first-order driver of gene content or a weak one. The tension statement itself attaches a caveat: the two results are not measured on the same thing, because these use different units, environments, and response variables [src: harvard_forest_warming; ecotype_env_reanalysis]. The conflict may therefore be one of comparability rather than of biology. Recording it here keeps both directions intact rather than collapsing them into a single verdict.
