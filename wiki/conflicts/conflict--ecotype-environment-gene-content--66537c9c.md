...
**Side B — the gene-content correlation does not follow.** The ecotype reanalysis found no stronger environmental than human-associated gene-content correlation — a null result on that comparison, not a weaker-but-present difference — and reported median partial correlation 0.0025 (the correlation between environment similarity and gene-content similarity after controlling for other terms) in the prior AlphaEarth-based analysis [src: env_embedding_explorer; ecotype_env_reanalysis]. That 0.0025 is reported for the prior AlphaEarth-based analysis, not for the environmental subset that carries the 3.4x gradient.
...
- *Hypothesis — different quantities.* The two results may be compatible because they are not measurements of the same thing: Side A reports a ratio in embedding distance, Side B a partial correlation between environment similarity and gene-content similarity, and a gradient in the first need not produce a gradient in the second.
