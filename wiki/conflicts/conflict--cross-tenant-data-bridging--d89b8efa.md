...
# Tryptophan: A Metabolite No Strain Grew On, Yet the Best-Predicted Growth Phenotype
...
**Binary growth on tryptophan is predictable from gene content.** Binary growth could be predicted for tryptophan with an AUC of 0.933 — AUC being the area under the receiver-operating-characteristic curve, a rank-based measure of how well a classifier separates growth cases from no-growth cases — ahead of phenylalanine (0.932) and valine (0.927). The same models produced negative R² for continuous phenotypes, R² being the fraction of variance explained, so a negative value means the fitted model explained less variance than simply predicting the mean; the predictability claim is therefore specific to the binary grow/no-grow call. [src: genotype_to_phenotype_enigma]

## Possible Reconciliations

- *Hypothesis: different denominators.* The null is within one species panel of 50 *P. fluorescens* strains [src: fw300_metabolic_consistency], while the AUC 0.933 is a cross-condition classification result [src: genotype_to_phenotype_enigma]; a classifier can separate grow from no-grow across a broader strain pool while a single clade is uniformly negative.
