...
# Are stress conditions where dark-gene phenotypes concentrate, or are stress phenotypes relatively depleted among the genes that resist reannotation?

Two projects in this corpus describe the condition profile of strong fitness phenotypes among unannotated ("dark") bacterial genes in opposite directions. One reports stress as the condition class where strong dark-gene phenotypes concentrate most; the other splits the dark set into annotation-lag genes (hypothetical in the original annotation but resolvable by modern reannotation) and truly dark genes (the residual genes that resist reannotation), and reports strong-phenotype truly dark genes as *depleted* in stress relative to annotation-lag genes. The disagreement matters for experimental prioritization: it determines whether a laboratory screening a dark-gene candidate list should weight stress conditions heavily, or whether stress is a poorer bet for the residual genes than the pooled result implies. Because the two analyses use different gene sets and different comparisons — a within-set concentration versus a between-class contrast — collapsing them into a single recommendation would misdirect assay design. The tension is recorded on [[concepts/experimental-prioritization-of-functional-dark-matter]].
