...
Pangenome openness — the propensity of a species' gene repertoire to keep expanding as more genomes are sampled — is assigned different determinants by different projects in this corpus, and the projects do not agree on how much any of them explains. One line of work treats variable metabolic capability as a reproducible candidate determinant or proxy of openness; another rests on null correlations between openness and the classic environment-versus-phylogeny axis; a third argues the environmental associations are coverage-limited and cannot be read as population-wide. This page records the disagreement carried on [[concepts/pangenome-openness-determinants]].
...
**The null correlations on environment and phylogeny.** On this side the openness–environment and openness–phylogeny correlations were null, and the HGT (horizontal gene transfer) and core/accessory explanations remain hypotheses requiring functional and transfer-specific tests, not established consequences of the null correlations. [src: pangenome_openness]
