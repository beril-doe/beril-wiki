...
Two lines of evidence in this corpus speak to whether pangenome structure carries ecological signal. Pangenome openness — how much a species' gene repertoire keeps growing as more genomes are sampled — shows null correlations with species-level environment and phylogeny effect sizes. [src: pangenome_openness] Separately, analyses contrasting core genes (present across the genomes of a sampled clade) with auxiliary genes (present in only some) find a conservation gradient that is statistically robust but only weakly predictive of fitness importance. [src: fitness_effects_conservation] The tension text states that the openness null does not contradict those within-clade findings but refines their scope, because the two operate on different units of analysis. [src: pangenome_openness; conservation_vs_fitness; fitness_effects_conservation] What stays unresolved — and shapes how core/auxiliary status should be read on [[concepts/pangenome-core-boundary-and-clade-size-bias]] — is whether openness fails to predict ecological dominance because the metric is too coarse, because relevant adaptation is concentrated in particular functional categories, or because of limited power. [src: pangenome_openness]
...
**Conservation tracks fitness importance within clades, but only weakly.** The within-clade conservation analyses compare gene categories inside sampled clades rather than across species, and design choices about which and how many genomes represent a clade can make the core boundary more or less discriminative. [src: conservation_vs_fitness] The conservation gradient itself is statistically robust and only weakly predictive of fitness importance. [src: fitness_effects_conservation] The tension text characterizes the relation to the openness null as refinement rather than contradiction: the null concerns species-level effect sizes, while the conservation work compares gene categories within sampled clades. [src: pangenome_openness; conservation_vs_fitness; fitness_effects_conservation]
