...
The corpus places two kinds of observation side by side without settling how they relate. Pathway completeness is nearly uniform across a small set of mapped organisms, yet the same corpus states that Web of Microbes production matches cannot establish uptake, pathway completeness, or gene essentiality, and that measured essentiality is predominantly organism-specific rather than universal. The disagreement matters because it decides whether pathway catalogues and metabolite-production records can be read as dependency evidence in [[concepts/gene-essentiality]], or only as capability inventories that require perturbation data before any essentiality claim is made.

## Evidence Sides

**Pathway completeness is near-uniform across the mapped organisms.** 17 of 18 amino-acid pathways were complete in all 7 mapped organisms [src: essential_metabolome]. This is the completeness observation at issue: a genome-level presence call, reported for a small organism sample, with one exception noted below.

**Observation of production cannot establish uptake or essentiality.** In the Web of Microbes snapshot — metabolite observations scored relative to a starting medium — there are no organism consumption actions, so consumption, and therefore uptake, is simply not recorded [src: webofmicrobes_explorer]. Chemical identity is also ambiguous: 107 formula-only matches (matches made on molecular formula alone, without structure) expanded to 900 candidate molecules [src: webofmicrobes_explorer]. On this side, production matches cannot establish uptake, pathway completeness, or gene essentiality.
