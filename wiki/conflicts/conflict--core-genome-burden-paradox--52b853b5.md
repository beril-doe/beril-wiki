...
Two sets of figures in this corpus quantify how much more often essentiality-associated genes fall in the *core* genome — the set of genes present across effectively all genomes of a clade's pangenome — and they report gaps of very different size against different comparison classes. Both point the same direction, so the disagreement is not about sign but about magnitude, and about whether that magnitude is a property of bacteria or of the analysis. It matters because a gap's width is what a core/auxiliary boundary would have to carry if it were read as a graded correlate of viability requirements, and the differing effect sizes leave the magnitude sensitive to dataset composition and operational definitions. [src: conservation_vs_fitness] The two results are directionally consistent but differ in category definitions and organism sets; neither resolves the poor binary agreement between insertion and complete-knockout assays. [src: fitness_effects_conservation, conservation_vs_fitness] The question runs through [[concepts/core-genome-burden-paradox]], [[concepts/essentiality-assay-discordance]] and [[concepts/pangenome-core-boundary-and-clade-size-bias]].
