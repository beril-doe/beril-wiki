...
Two projects in this corpus evaluate the same organism, *Acinetobacter baylyi* ADP1, against the same class of model output — flux balance analysis (FBA), a constraint-based method that predicts feasible metabolic flux distributions and, from them, whether a gene is needed for simulated growth — and reach readings that point in different directions. One reports substantial agreement between FBA and TnSeq (transposon sequencing, which infers gene essentiality from depletion of transposon insertion mutants) essentiality calls; the other reports moderate agreement with experimental knockouts together with a null association between FBA class and growth defects within TnSeq-dispensable genes. Because gapfilled-model predictions are here being compared directly against experimental essentiality data (see [[concepts/metabolic-model-gapfilling]]), how far FBA agreement actually extends in this setting determines how much downstream inference a gapfilled model can carry.
...
- Stratify the paired FBA/TnSeq calls by FBA class and by TnSeq call and recompute agreement within each stratum: which strata carry the 73.8% figure, and do the discordant genes cluster in one of them? [src: acinetobacter_adp1_explorer]
