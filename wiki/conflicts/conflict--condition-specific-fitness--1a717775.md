...
Four analyses in this corpus report how often genes required for growth under metal stress belong to the **core genome** (the gene set shared by essentially all genomes of a species pangenome) rather than the accessory genome, and they do not agree. A cross-organism metal atlas reports core *enrichment* above baseline, at 87.4% core versus a 76.9% baseline. A single-organism condition-specific analysis reports heavy-metal genes at 71.2% core and **not** significantly enriched after correction for testing many categories at once — a null result, which stays null. A specificity analysis reports its own core fractions for similarly named gene classes [src: metal_specificity], and a three-tier cross-resistance analysis reports a fourth value for a similarly named class [src: metal_cross_resistance, metal_fitness_atlas]. This matters because the readings imply opposite evolutionary stories — metal survival as a conserved cellular-vulnerability problem versus metal resistance as recently acquired accessory machinery — and because the numbers are close enough to invite an illegitimate average. They must not be averaged: definitions, thresholds, organism sets, general-stress capture, exclusion of putatively essential genes, and stress histories differ. [src: metal_fitness_atlas] [src: field_vs_lab_fitness] [src: metal_specificity]
