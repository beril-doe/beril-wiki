...
**Significant but confounded — PHB and niche breadth.** PHB (polyhydroxybutyrate, a bacterial carbon-storage polymer) showed a raw breadth association of rho=0.106 (Spearman rank correlation, a monotonic association measure) at p=1.77×10^-06. [src: phb_granule_ecology] The genome-size-controlled partial rho — the same correlation with genome size held constant — was -0.047 at p=0.037, a sign reversal leaving the controlled estimate pointing the opposite way. [src: phb_granule_ecology] Here the raw association is attributable to a covariate, and the controlled estimate is near-null with an inverted direction rather than merely attenuated.
