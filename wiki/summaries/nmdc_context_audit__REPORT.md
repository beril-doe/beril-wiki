...
The freshest NMDC resource is `kbase.nmdc_mags`, containing 62,346 MAGs, but it is located in the tenant users are least likely to search when looking for NMDC data. [src: nmdc_context_audit]
...
The audit recommends surfacing `max(committed_at)` in discovery tooling because tables have no comments, databases have empty properties, and no changelog exposes currency. [src: nmdc_context_audit]
