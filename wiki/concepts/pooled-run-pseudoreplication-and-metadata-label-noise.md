...
This issue is directly relevant to [[concepts/cross-tenant-data-bridging]], because the validity of ecological inference depends on joining workflow results to the correct metadata grain. [src: euk_in_prok_correlates] It also refines [[concepts/cross-tenant-data-bridging]]: successful key-based joins do not by themselves demonstrate that a pooled run has a uniquely correct environmental label. [src: euk_in_prok_correlates] The problem contributes to [[concepts/environmental-resistome]], since metadata recorded at one biosample or representative-biosample level may not describe all material contributing to a pooled run. [src: euk_in_prok_correlates]
