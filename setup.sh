...
#   ./setup.sh          then: node pipeline/serve_quartz.cjs
# Needs: uv (python env), Bun (dependencies) and Node (Quartz). Viewing requires no API keys
...
command -v bun >/dev/null || { echo "missing bun — install with:  brew install bun"; exit 1; }

uv sync --project "$HERE"
"$HERE/pipeline/build_quartz.sh"
echo
echo "setup done. view the wiki with:"
echo "  node $HERE/pipeline/serve_quartz.cjs"
