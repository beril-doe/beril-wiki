#!/usr/bin/env python3
"""Copy the wiki pages into a Quartz content/ dir.
...
Usage: publish/ingest.py <kb-root> <quartz-content-dir>
"""
...
from beril_wiki import evidence
...
# carries the computed evidence terms from src/beril_wiki/evidence.py when the page
# has any.
...
PROVENANCE = (
    "AI-generated wiki from AI-conducted research reports. Partially "
...
COLLECTION_INDEX = {
    ...
    "sources": ("Raw reports", "The raw research reports this wiki is compiled from, unedited."),
}
...
def copy_figure(src: pathlib.Path, dst: pathlib.Path) -> None:
    ...
    if src.stat().st_size > BIG_FIGURE and shutil.which("sips"):
        subprocess.run(
            ["sips", "-Z", "1600", str(src), "--out", str(dst)], check=False, capture_output=True
        )
        if dst.exists():
            return
...
def split_paragraphs(body: str) -> list[str]:
    """Raw paragraph chunks; same inclusion rule as stages.figures.paragraphs
...
def splice_figures(text: str, entry: dict, kb: pathlib.Path, dst_root: pathlib.Path) -> str:
    if hashlib.sha256(text.encode()).hexdigest()[:16] != entry.get("page_hash"):
        return text  # page changed since placement; stages.figures will re-run
    m = FM.match(text)
    ...
    for pl in sorted(entry["placements"], key=lambda p: -p["after_paragraph"]):
        ...
        block = (
            f"![{pl['caption']}](../figures/{pl['project']}/{name})\n"
...
def rewrite_source_figures(
    text: str, project: str, kb: pathlib.Path, dst_root: pathlib.Path
) -> str:
    def repl(m: re.Match) -> str:
...
def main() -> None:
    ...
    targets = {
        ...
        if not (f.parent.name == "entities" and f.stem in skip_entities)
    }
    ...
    for src in root.rglob("*.md"):
        ...
        if src.name in SKIP or rel.parts[0] == "reports":
            continue
        if rel.parts[0] == "entities" and src.stem in skip_entities:
            continue
        pub = published(rel)
        out = dst / pub
        ...
        # judged: see src/beril_wiki/evidence.py on why it is not the v1 atlas's
        ...
        if rel.parts[0] == "summaries":
            ...
            if rel.stem.endswith("__REPORT"):
                text = re.sub(
                    r'^type: "Summary"$', 'type: "Project report"', text, count=1, flags=re.M
                )
    ...
    # and anything else the wiki needs to ship alongside its prose.
    assets = root / "assets"
    if assets.is_dir():
