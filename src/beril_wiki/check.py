...
def prose_only(par: str) -> str:
    ...
    # HTML comments are machinery, not prose: stages.conflicts stamps a
    ...
    text = WIKILINK.sub(
        lambda m: m.group(0).split("|", 1)[1][:-2] if "|" in m.group(0) else " ",
        SRC_TAG.sub("", par),
    )
...
def derivable_numbers(par: str, ids: list[str], sources: dict[str, str]) -> set[str]:
    ...
    pool = {
        v
        for v in (_as_float(t) for s in known for t in source_numbers(s, sources[s]))
        if v is not None
    }
    out = set()
...
def paragraphs(body: str) -> list[str]:
    ...
    # stages/literature.py), so their numbers are exempt from corpus-source checks.
    body = re.sub(r"^---\n.*?\n---\n", "", body, flags=re.S)
    ...
    # adjacent reason — it cites external PMIDs, verified by stages/literature.py.
    body = re.sub(
        r"^## (Literature Context|Open Directions|Resolving Work|Possible Reconciliations)"
        r"\s*\n.*?(?=\n## |\Z)",
        "",
        body,
        flags=re.M | re.S,
...
NOT_DUPLICATES = {
    ...
    frozenset(
        {"horizontal-gene-transfer-driven-innovation", "chromosomal-and-integrative-gene-transfer"}
    ),
...
def duplicate_concepts(kb: pathlib.Path) -> list[str]:
    ...
    Shared with compiler.py's plan step, which injects the current output so
    ...
    for i, (a, sa, ta) in enumerate(cinfo):
        for b, sb, tb in cinfo[i + 1 :]:
            ...
            if sa and sb and len(sa & sb) / max(1, len(sa | sb)) >= 0.5 and len(ta & tb) >= 2:
                out.append(
                    f"duplicate-concepts? '{a}' and '{b}' share {len(sa & sb)} sources "
                    f"and name tokens {sorted(ta & tb)}"
                )
    return out
...
def main() -> int:
    ...
    kb = pathlib.Path(argv[0]) if argv else ROOT
    sources = source_ids(kb)
    if not sources:
        print(f"check: no sources found under {kb}/staging or {kb}/raw", file=sys.stderr)
        return 1

    # --strict promotes numeric mismatches and dead links from warning to error.
    # The pre-existing violations this waited on are repaired (stages/repair.py),
    ...
    roots = [
        (wiki, sub)
        for sub in ("concepts", "entities", "summaries", "topics", "conflicts", "authors")
    ]
    for base, sub in roots:
        for page in sorted((base / sub).glob("*.md")):
            ...
            for i, par in enumerate(
                paragraphs(page.read_text(encoding="utf-8", errors="replace")), 1
            ):
                ids = cited_ids(par)
                ...
                if nums and not ids:
                    warns.append(
                        f"{rel} ¶{i}: {len(nums)} figure(s) but no [src:] citation: {par[:90]!r}"
                    )
                    continue
                if nums and ids:
                    n_numeric_pars += 1
    ...
    for base, sub in [(kb / "wiki", "concepts"), (kb / "wiki", "entities")]:
        for page in sorted((base / sub).glob("*.md")):
            ...
            if not m:
                continue
            listed = {
                re.sub(r"__REPORT$", "", s.rsplit("/", 1)[-1].removesuffix(".md"))
            ...
            for extra in sorted(listed - cited):
                msg = (
                    f"{sub}/{page.name}: frontmatter lists source {extra!r} "
                    "that the body never cites"
                )
                (errors if strict else warns).append(msg)

    # Dead [[wikilinks]]. compile.generate_page validates targets at write time,
    # but stages.topics, stages.conflicts and stages.authors use their own llm() and
    # never did, so their pages accumulated links to concepts that no longer
    # exist. publish.ingest downgrades them at publish, which hid the problem
    # rather than fixing it.
    targets = {str(f.relative_to(wiki)).removesuffix(".md") for f in wiki.rglob("*.md")}
    for base, sub in roots:
        for page in sorted((base / sub).glob("*.md")):
            text = page.read_text(encoding="utf-8", errors="replace")
            dead = sorted(
                {m.group(1).strip().lstrip("/") for m in WIKILINK.finditer(text)} - targets
            )
            for d in dead:
    ...
    for base, sub in [(wiki, "concepts"), (wiki, "topics")]:
    ...
    for p in sorted(proj_ids):
        if uptake.get(p, 0) < 2:
            warns.append(
                f"uptake: project '{p}' cited by only {uptake.get(p, 0)} concept/topic "
                "page(s) — under-integrated"
            )

    warns.extend(duplicate_concepts(kb))

    print(
        f"check: {n_pages} pages, {n_cited_pars} cited paragraphs, "
    ...
    if n_derived:
        print(
            f"  NOTE  {n_derived} figure(s) not verbatim in a cited source but derivable from "
            f"two of its figures (a difference or ratio) — accepted, not verified"
        )
    print(f"check: {len(errors)} error(s), {len(warns)} warning(s), {n_derived} derived")
