...
Output goes to wiki/{authors,data}/ next to the compiled collections.

    uv run python -m beril_wiki.stages.extras
"""
...
from beril_wiki.stages.people import build_author_index
...
def wiki_projects() -> list[str]:
    ...
    return sorted(re.sub(r"__REPORT$", "", f.stem) for f in (ROOT / "staging").glob("*__REPORT.md"))
...
def write_authors() -> int:
    ...
    for record in index.values():
        ...
        # Carry the LLM-written Contributions section (stages/authors.py) across
        # regenerations. "Profile" is the pre-rev-2 heading: matched so old
        # stubs still parse, never re-emitted — stages.authors rewrites them.
        profile = ""
        if path.exists():
            m = re.search(
        ...
        if kind := NON_HUMAN.get(slugify(record.name)):
            lines += [f"*Not a person — {kind}. Listed as an author on the projects below.*", ""]
        if record.orcid:
...
def write_collections() -> int:
    ...
    for proj in projects:
        text = (ROOT / "staging" / f"{proj}__REPORT.md").read_text(
...
def write_opportunities() -> int:
    ...
    for page in sorted((ROOT / "wiki/concepts").glob("*.md")):
        ...
        lines += [
            f"## [[concepts/{page.stem}|{h1.group(1).strip() if h1 else page.stem}]]",
            "",
            m.group(1).strip(),
            "",
        ]
...
def write_negative_results() -> int:
    ...
    for page in sorted((ROOT / "wiki/summaries").glob("*.md")):
        ...
        h1 = re.search(r"^# (.+)$", text, re.M)
        lines += [
            f"## [[summaries/{page.stem}|{h1.group(1).strip() if h1 else page.stem}]]",
            "",
            "\n\n".join(blocks),
            "",
        ]
...
if __name__ == "__main__":
    ...
    write_indexes()
    print(
        f"wrote {n_a} author pages, {n_c} collection pages, opportunities ({n_o} concepts), "
