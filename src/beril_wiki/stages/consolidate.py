...
replacing check's name-token duplicate heuristic for candidate generation:
...
from beril_wiki.check import NUMBER, cited_ids, norm_num, paragraphs, source_ids
...
MERGE_MODELS = os.environ.get(
    "MERGE_EMBED_MODELS", "openai/lbl/nomic-embed-text,openai/cohere-embed-v4"
).split(",")
...
def embed(texts: list[str], model: str = EMBED_MODEL) -> list[list[float]]:
    ...
    if len(left) > max(4, len(texts) // 20):
        raise SystemExit(
            f"[ERROR] {model} returned {len(left)} duplicate vectors for distinct "
            f"inputs after 3 repair passes — refusing to rank on corrupt embeddings"
        )
    print(
        f"    [WARN] {model}: {len(left)} vector(s) still duplicated "
...
def _collisions(texts: list[str], raw: list[list[float]]) -> list[int]:
    ...
    return [
        ...
        if len(g) > 1 and len({texts[k][:EMBED_CHARS] for k in g}) > 1
        for i in g
    ]
        ...
        out.append(
            {
                ...
                "body": body,
                "digest": digest(text),
                "etext": f"{sid_of(p.stem).replace('_', ' ')}. "
...
def same_source_candidates(concepts: list[dict], mature: int) -> list[tuple]:
    ...
    `stages/enrich.py` audits one summary at a time and may create several
...
def merge_candidates(
    concepts: list[dict], vecs_by_model: dict[str, list], topn: int, mature: int
) -> list[tuple]:
...
def rewrite_concept_links(
    text: str, loser: str, survivor: str, collection: str = "concepts"
) -> str:
    ...
    out = re.sub(
        ...
        lambda m: f"[[{collection}/{survivor}{m.group(1)}]]",
        text,
    )
    if out == text:
        return text
...
def repoint_links(
    root: pathlib.Path, loser: str, survivor: str, collection: str = "concepts"
) -> int:
    changed = 0
...
def generate_retaining(
    messages: list[dict],
    step: str,
    sources: dict[str, str],
    targets: set[str],
    required: set[str],
    required_nums: set[str],
    ...
    and every gate in the pipeline passed, because check validates the
    ...
    def losses(obj: dict) -> tuple[list[str], list[str]]:
        ...
        if unchanged_ok and content == UNCHANGED:
            return [], []
        return (
            sorted(required - body_src_ids(content)),
    ...
    def violations(ids: list[str], nums: list[str]) -> str:
        ...
        if nums:
            v.append(
                f"- you deleted {len(nums)} figure(s) that appear in the page(s) above: "
    ...
    obj = C.generate_page(
        ...
        + [
            ...
            {
                "role": "user",
                "content": C.RETRY_USER.format(violations=violations(lost_ids, lost_nums)),
            },
        ],
        f"{step}/retention",
        sources,
    ...
    if lost_ids or lost_nums:
        raise C.PageError(
            f"{step}: still dropping citations {lost_ids} / figures {lost_nums[:8]} after retry"
        )
    return obj
...
def replay_pending(root: pathlib.Path, state: dict, state_path: pathlib.Path) -> None:
    ...
    for key, survivor in list(state.get("pending", {}).items()):
        ...
        print(
            f"  resuming interrupted merge {collection}/{loser} -> {survivor}: "
...
def apply_merge(
            ...
            [
                {"role": "system", "content": system},
                {
                    ...
                    "content": C.KNOWN_TARGETS_USER.format(
                        targets="\n".join(
                            f"- {t}" for t in sorted(targets - {f"{collection}/{loser}"})
                        )
                    ),
                },
    ...
    srcs = C.canonical_sources(
        obj["content"], list(dict.fromkeys((sfm.get("sources") or []) + (lfm.get("sources") or [])))
    )
    ps.write_text(
...
def apply_decisions(
    root: pathlib.Path, sources: dict[str, str], system: str, state: dict, state_path: pathlib.Path
) -> list[str]:
    ...
    for m in d["merges"]:
        ...
        if not apply_merge(root, loser, survivor, targets, sources, system, state, state_path, []):
    ...
    for rank, sim, i, j in pairs:
        ...
        raw = C.llm(
            [
                ...
                {
                    "role": "user",
                    "content": MERGE_JUDGE_USER.format(
                        a=a,
                        b=b,
                        body_a=ba,
                        body_b=bb,
        ...
        print(
            f"  merging concepts/{loser} -> concepts/{survivor} (rank {rank}, sim {sim:.3f}): "
            f"{str(verdict.get('justification') or '')[:90]}"
        )

        if not apply_merge(
            root, loser, survivor, targets, sources, system, state, state_path, refused
        ):
            continue
    ...
    for sim, i, j in cands:
        ...
        try:
            obj = generate_retaining(
                ...
                f"backmerge/{c['stem']}+{s['sid']}",
                sources,
                targets,
                required=body_src_ids(text),
                required_nums=page_numbers(text),
                unchanged_ok=True,
            )
        except C.PageError as e:
...
def report(
    concepts: list[dict], summaries: list[dict], cvecs, pairs: list[tuple], cands: list[tuple], args
) -> None:
    hist: dict[int, int] = {}
    for c in concepts:
    ...
    for rank, sim, i, j in pairs:
        print(
            f"    #{rank:<3d} {sim:.3f}  {concepts[i]['stem']} ({len(concepts[i]['cited'])})"
...
def main(root: pathlib.Path, args) -> int:
    ...
    if args.rename or args.force_merge:
        ...
        print(
            f"consolidate: recorded and applied decisions (~${est:.2f} est); "
    ...
    print(
        f"consolidate: embedding {len(concepts)} concepts + {len(summaries)} summaries "
        f"({', '.join(models)})"
    )
    ...
    ev = [
        (0, j, i, k)
        for j, _, i, k in evidence_candidates(
            concepts, args.min_shared_numbers, args.evidence_jaccard
        )
    ...
    ss = [
        (1, float(n), i, j)
        for n, i, j in same_source_candidates(concepts, args.min_sources)
        if (i, j) not in seen
    ]
    seen |= {(i, j) for _, _, i, j in ss}
    emb = [
        p
        for p in merge_candidates(concepts, cvecs_by, args.merge_topn, args.min_sources)
        if (p[2], p[3]) not in seen
    ]
    pairs = ev + ss + emb
    print(
        f"  merge candidates: {len(ev)} evidence-overlap + {len(ss)} same-source + "
        f"{len(emb)} embedding = {len(pairs)}"
    )
    cands = backmerge_candidates(
        concepts, cvecs, summaries, svecs, args.backmerge_threshold, args.topk, args.min_sources
    )
    ...
    refused: list[str] = []
    merged = phase_merge(
        root, concepts, pairs, sources, system, state, args.min_sources, state_path, refused
    )
    state_path.write_text(json.dumps(state, indent=1, sort_keys=True))
    ...
    added, skipped = phase_backmerge(
        root, concepts, summaries, cands, sources, system, state, refused
    )
    state_path.write_text(json.dumps(state, indent=1, sort_keys=True))
    ...
    est = C._usage["in"] * C.PRICE_IN + C._usage["out"] * C.PRICE_OUT
    print(
        f"consolidate: {merged} merge(s), {added} back-merge(s), {skipped} no-evidence, "
