...
TARGETS = (
    ("wiki", "*.md"),  # home, catalog, log, about and the digests
    ("wiki/concepts", "*.md"),
    ("wiki/entities", "*.md"),
    ("wiki/summaries", "*.md"),
    ("wiki/topics", "*.md"),
...
def protect(line: str) -> bool:
    ...
    return (
        ...
        or (
            ":" in s.split(" ")[0]
            and s.split(":")[0] in {"type", "sources", "full_text", "doc_type"}
        )
    )
...
# but the hash no longer matches and publish.ingest drops every figure on that
...
def placement_key(root: pathlib.Path, path: pathlib.Path) -> str:
    """The key the figures stage stores: <collection>/<name>.md, relative to wiki/."""
...
def main() -> int:
    ...
    print(
        f"stages.names: {changed} line(s) renamed across {touched} file(s); "
