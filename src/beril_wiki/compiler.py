...
oldest-first by project id):
  ...
  5 catalog.md + log.md maintained deterministically in code.
...
PRICE_OUT = float(
    os.environ.get("COMPILE_PRICE_OUT", "1.5e-6")
)  # override when running Sonnet (3e-6/15e-6)
...
def llm(messages: list[dict], step: str) -> str:
    ...
    if configured():
        mechanical = step.rsplit("/", 1)[-1] in ("plan", "queries", "enrich-plan") or (
            step.startswith("merge-judge/")
        )
        return text_completion(messages, step, review=not mechanical)
    est = _usage["in"] * PRICE_IN + _usage["out"] * PRICE_OUT
    if est > BUDGET_USD:
        raise SystemExit(
            f"[ERROR] budget cap ${BUDGET_USD:.2f} exceeded (est ${est:.2f}) — "
            f"stopping before {step}"
        )
    resp = completion(
        model=MODEL,
...
def wikilink_targets(root: pathlib.Path) -> set[str]:
    wiki = root / "wiki"
    return {str(f.relative_to(wiki)).removesuffix(".md") for f in wiki.rglob("*.md")}
...
def validate_page(
    ...
    for i, par in enumerate(paragraphs(content), 1):
        ...
        if nums and not ids:
            ...
            v.append(
                f"paragraph {i}: contains figure(s) {nums[:4]} but no [src:] citation: {par[:80]!r}"
            )
        elif nums and known:
...
def prose_violations(page: str, sources: dict[str, str]) -> list[str]:
    ...
    stages.topics, stages.conflicts and stages.authors build their pages with their
    ...
    def attempt(msgs: list[dict], name: str) -> tuple[str, dict | None, list[str]]:
        ...
        try:
            ...
            return (
                ...
                None,
                [
                    f"your reply was not parseable JSON ({e}) — "
                    "resend the SAME page as one valid JSON object"
                ],
            )
        return (
            raw,
            obj,
            validate_page(obj.get("content") or "", sources, targets, require_slots, check_links),
        )
        ...
        for kind in ("create", "update"):
            for it in g.get(kind) or []:
                if (
                    ...
                    or not isinstance(it.get("name"), str)
                    or not it["name"].strip()
                ):
                    continue
                ...
                if exists:
                    ...
                    print(
                        f"    [WARN] plan updated nonexistent {group[:-1]} {name!r} — "
                        "treating as create"
                    )
                    out[group]["create"].append(item)
    return out
...
def compile_doc(root: pathlib.Path, fname: str, sources: dict[str, str], system: str) -> None:
    ...
    def whitelist_msg(tg: set[str]) -> dict:
        return {
            "role": "user",
            "content": KNOWN_TARGETS_USER.format(targets="\n".join(f"- {t}" for t in sorted(tg))),
        }
    ...
    summary_obj = generate_page(
        [
            ...
            whitelist_msg(targets),
            {
                "role": "user",
                "content": SUMMARY_USER.format(fname=fname, sid=sid, content=sources[sid]),
            },
        ],
        f"{sid}/summary",
        sources,
    ...
    (wiki / "summaries" / f"{stem}.md").write_text(
        fm_block(
            {
                "type": "Summary",
                "description": summary_obj.get("description", ""),
                "doc_type": "short",
                "full_text": f"sources/{fname}",
            }
        )
    ...
    summary_ctx = {
        "role": "user",
        "content": f"Summary of the new document {fname} (source id {sid}):\n\n{summary_md}",
    }
    jobs = [("concepts", it) for it in plan["concepts"]["update"] + plan["concepts"]["create"]] + [
    ...
    for group, it in jobs:
        ...
        old_fm, old_body = (
            parse_fm(path.read_text(encoding="utf-8", errors="replace"))
            if path.exists()
            else ({}, "")
        )
        ...
        if group == "concepts":
            task = (
                CONCEPT_UPDATE_USER.format(
                    title=title, name=name, sid=sid, existing=old_body, summary_stem=stem
                )
                if old_body
                else CONCEPT_CREATE_USER.format(title=title, name=name, sid=sid, summary_stem=stem)
            )
        else:
            etype = it.get("type") or str(old_fm.get("type") or "other").lower()
            task = (
                ENTITY_UPDATE_USER.format(
                    title=title,
                    name=name,
                    sid=sid,
                    etype=etype,
                ...
                if old_body
                else ENTITY_CREATE_USER.format(
                    title=title,
                    name=name,
                    sid=sid,
                    etype=etype,
        ...
        try:
            ...
            print(
                f"    [ERROR] {e} — keeping old version"
                if old_body
                else f"    [ERROR] {e} — page not created"
            )
        ...
        if group == "entities":
            etype = (
                obj.get("type") if obj.get("type") in ENTITY_TYPES else (it.get("type") or "other")
            )
            fm["type"] = fm_entity_type(str(etype))
...
def main(root: pathlib.Path, only: list[str] | None = None) -> int:
    ...
    if not sources:
        ...
        return 1
    system = SYSTEM.format(contract=(ROOT / "contract" / "AGENTS.md").read_text(encoding="utf-8"))
    ...
    for fname in docs:
        ...
        if len(_failures) > n_fail_before:
            ...
            print(
                f"    [ERROR] {fname}: {len(_failures) - n_fail_before} page(s) rejected — "
                "doc left dirty for re-run"
            )
            continue
        hashes[fname] = digest
    ...
    est = _usage["in"] * PRICE_IN + _usage["out"] * PRICE_OUT
    print(
        f"compile: {done} doc(s) compiled, {skipped} unchanged, {len(_failures)} failure(s); "
